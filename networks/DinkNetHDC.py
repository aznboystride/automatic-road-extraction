...
class _DenseUpsamplingConvModule(nn.Module):
    def __init__(self, down_factor, in_dim, num_classes=1):
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
'''class DecoderBlock(nn.Module):
    def __init__(self, in_channels, n_filters, upsample=True):
        super(DecoderBlock,self).__init__()
...
class DinkNetHDC(nn.Module):
    def __init__(self, num_classes=1, num_channels=3):
        ...
        '''layer4_group_config = [2,4,2]'''
        for idx in range(len(self.encoder4)):
            self.encoder4[idx].conv2.dilation = (2, 2)
            self.encoder4[idx].conv2.padding = (2, 2)

        #self.jpu = JPU([128,256,512], norm_layer=nn.BatchNorm2d, up_kwargs={'scale_factor': None})
        ...
        #self.skipcnv1 = nn.Conv2d(128,256,3,padding=1)
        #self.skipcnv2 = nn.Conv2d(64,128,3,padding=1)
        self.skipcnv1 = nn.Conv2d(filters[2] + filters[1], filters[2] + filters[1],1)
        self.skipcnv2 = nn.Conv2d(filters[1] + filters[0], filters[1] + filters[0],1)
        self.finaldeconv1 = nn.ConvTranspose2d(filters[0], 32, 4, 2, 1)
        self.finalrelu1 = nonlinearity
        self.finalconv2 = nn.Conv2d(32, 32, 3, padding=1)
        self.finalrelu2 = nonlinearity
    ...
    def forward(self, x):
        ...
        # Decoder
        d3 = nonlinearity(self.skipcnv1(torch.cat((self.decoder3(e4), e2), 1)))# + self.skipcnv1(e2)
        d2 = nonlinearity(self.skipcnv2(torch.cat((self.decoder2(d3), e1), 1)))# + self.skipcnv2(e1) 
        d1 = self.decoder1(d2)
