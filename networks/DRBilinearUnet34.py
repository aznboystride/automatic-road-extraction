...
class DRBilinearUnet34(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.d4 = nn.Sequential(
            nn.Conv2d(filters[-1], filters[-1], 3, stride=1, padding=1, dilation=1),
            nn.BatchNorm2d(filters[-1]),
            nn.ReLU()
        )

        self.d3 = nn.Sequential(
            nn.Conv2d(filters[-2], filters[-2], 3, stride=1, padding=2, dilation=2),
        ...
        self.d1 = nn.Sequential(
            nn.Conv2d(filters[-4], filters[-4], 3, stride=1, padding=8, dilation=8),
            nn.BatchNorm2d(filters[-4]),
            nn.ReLU()
        )

        self.d0 = nn.Sequential(
            nn.Conv2d(filters[-4], filters[-4], 3, stride=1, padding=16, dilation=16),
            nn.BatchNorm2d(filters[-4]),
            nn.ReLU()
        )
    ...
    def upsample(self, ch_coarse, ch_fine):
        return nn.Sequential(
            nn.Conv2d(ch_coarse, ch_fine, 3, 1, 1),
